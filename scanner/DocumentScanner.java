...
public class DocumentScanner {
     ...
     */
    protected float shortRowFraction = .125f;
    /**
    ...
    /**
     * Scan every row in the array, <code>pixelImage</code>, bounded by the
     * dimensions of <code>pixelImage</code> or <code>blockX1</code>,
     * <code>blockX2</code>, <code>blockY1</code>, <code>blockY2</code>. For
     * those rows which are not complete white-space or those in a series of
     * rows of white-space contained within a character, record the beginning
     * and ending row of non-white-space. Feed these results into
    ...
    public final void scan(
        ...
        // blockX1 and blockY1 ==> 0; 
        // blockX2 and blockY2 ==> pixelImage width and height, respectively;
        if (blockX1 < 0) {
            blockX1 = 0;
        } else if (blockX1 >= w) {
            blockX1 = w - 1;
        ...
        // Adds row numbers which contain non-white space to array.
        for (int y = blockY1; y < blockY2; y++) {
            boolean isWhiteSpace = true;
            // If non-white space is found in any part of the row, isWhiteSpace
            // is false.
            ...
            // If only white space was found in the row and previous row(s)
            // had non-white space, track the span of rows the non-white
            // was found.
            if (isWhiteSpace) {
                if (!whiteLine) {
                    whiteLine = true;
            ...
            } // If non-white space was found and previous row was all white,
            // record this row - y - into y1.
            else {
                if (whiteLine) {
                    whiteLine = false;
                    y1 = y;
        ...
        // If last row wasn't all white, add the span of rows of non-whites to
        // array.
        if (!whiteLine) {
            al.add(new Integer(y1));
        ...
        for (int i = 0; (i + 4) <= al.size(); i += 2) {
            ...
            if (((row1H <= (int) ((float) row0H * shortRowFraction))
                    ...
                    || (whiteH  <= (int) ((float) row1H * shortRowFraction))
                    || (whiteH <= 15))) {
                al.remove(i + 2);
                al.remove(i + 1);
                i -= 2;
...
///System.err.println("process row: "+blockX1+","+bY1+" "+blockX2+","+bY2);
    ...
    private void processRow(
        ...
        for (int x = x1 + 1; x < (x2 - 1); x++) {
            ...
            // traverse down column at position x.
            ...
            if (isWhiteSpace) {
                ...
                // Non-white space found in column.    
            } else {
                numConsecutiveWhite = 0;
                if (inCharSeparator) {
        ...
        for (int i = 0; (i + 1) < al.size(); i += 2) {
            ...
            if (cy1 >= cy2) {
                ...
                // Does nothing with ImageScanner Class.
